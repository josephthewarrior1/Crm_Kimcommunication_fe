...
export const EngagementModal: React.FC<EngagementModalProps> = ({
  ...
  const handleLogActivity = async (type: 'CALL' | 'EMAIL' | 'WHATSAPP') => {
    ...
    try {
      ...
      toast.success(`Berhasil mencatat ${type} untuk ${participant.database.firstName}!`);
      setNotes('');
      ...
      >
        ...
        <div className="p-6 space-y-6 overflow-y-auto flex-1 text-slate-900">
          ...
          <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-4 space-y-2">
            ...
            <input
              type="text"
              placeholder="Misal: 'Sudah ditelpon, konfirmasi hadir acara' atau 'Email reminder terkirim'"
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              className="w-full px-3.5 py-2 text-xs bg-white border border-slate-200 rounded-xl focus:outline-none focus:border-blue-500 text-slate-800"
            />
          </div>
