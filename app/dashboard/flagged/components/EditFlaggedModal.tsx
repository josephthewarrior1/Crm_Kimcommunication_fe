...
import { ShieldAlert, User, FileText, Link2, Loader2, Save } from 'lucide-react';
        ...
        <header className="shrink-0 border-b border-slate-200 bg-white px-4 py-5 sm:px-6">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              <DialogTitle>Edit flagged identity</DialogTitle>
              <DialogDescription>Update contact information, review status, and supporting notes.</DialogDescription>
            </div>
          </div>
        </header>
        ...
        <form onSubmit={handleSubmit} className="ms-modal-form">
          <div className="min-h-0 flex-1 space-y-5 overflow-y-auto overscroll-contain bg-white p-4 sm:p-6">
            <fieldset className="min-w-0 space-y-4">
              ...
              <div>
                <label htmlFor="edit-flagged-name" className="mb-1.5 block text-sm font-medium text-slate-700">Name used</label>
                <input id="edit-flagged-name" type="text" placeholder="Enter the name used" value={nameUsed} onChange={event => setNameUsed(event.target.value)} className="w-full rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-100" />
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label htmlFor="edit-flagged-email" className="mb-1.5 block text-sm font-medium text-slate-700">Email used</label>
                  <input id="edit-flagged-email" type="email" placeholder="email@example.com" value={emailUsed} onChange={event => setEmailUsed(event.target.value)} className="w-full rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-100" />
                </div>
                <div>
                  <label htmlFor="edit-flagged-phone" className="mb-1.5 block text-sm font-medium text-slate-700">Phone used</label>
                  <input id="edit-flagged-phone" type="tel" placeholder="0812 3456 7890" value={phoneUsed} onChange={event => setPhoneUsed(event.target.value)} className="w-full rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-100" />
                </div>
              </div>
            </fieldset>
            ...
            <fieldset className="min-w-0 space-y-4 border-t border-slate-200 pt-5">
              ...
              <div className="clear-both grid gap-4 sm:grid-cols-2">
                <div>
                  <label htmlFor="edit-flagged-status" className="mb-1.5 block text-sm font-medium text-slate-700">Flag status</label>
                  <select id="edit-flagged-status" value={status} onChange={event => setStatus(event.target.value)} className="w-full rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-100">
                ...
                <div>
                  <label htmlFor="edit-flagged-reason" className="mb-1.5 block text-sm font-medium text-slate-700">Reason</label>
                  <select id="edit-flagged-reason" value={flagReason} onChange={event => setFlagReason(event.target.value)} className="w-full rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-100">
            ...
            <fieldset className="min-w-0 rounded-lg border border-slate-200 bg-slate-50 p-4">
              ...
              <p id="edit-flagged-database-hint" className="clear-both mb-4 text-xs leading-5 text-slate-500">Selecting a CRM profile fills in its contact information.</p>
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label htmlFor="edit-flagged-database" className="mb-1.5 block text-sm font-medium text-slate-700">CRM profile</label>
                  <select id="edit-flagged-database" aria-describedby="edit-flagged-database-hint" value={selectedDatabaseId} onChange={event => handleSelectDatabaseChange(event.target.value)} className="w-full rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-100">
                ...
                <div>
                  <label htmlFor="edit-flagged-event" className="mb-1.5 block text-sm font-medium text-slate-700">Event</label>
                  <select id="edit-flagged-event" value={selectedEventId} onChange={event => setSelectedEventId(event.target.value)} className="w-full rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-100">
