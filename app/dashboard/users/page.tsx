        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-white border border-slate-200 rounded-2xl shadow-xl relative animate-in scale-in duration-200 text-slate-900">
            ...
            <form
              onSubmit={async (e) => {
                e.preventDefault();
                if (!resetPasswordVal.trim() || !resetConfirmPasswordVal.trim()) {
                ...
                try {
                  await crmService.updateUserPassword(changingPasswordUser.id, resetPasswordVal.trim());
            ...
            >
              <div className="text-xs text-slate-500 bg-slate-50 border border-slate-100 rounded-xl p-3 mb-2">
                You are updating the password for user <span className="font-extrabold text-slate-800">@{changingPasswordUser.username}</span> ({changingPasswordUser.fullName || changingPasswordUser.email}).
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <button
                    type="button"
                    onClick={() => setShowResetPassword((val) => !val)}
                    className="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-slate-600 transition-colors"
                  >
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <button
                    type="button"
                    onClick={() => setShowResetConfirmPassword((val) => !val)}
                    className="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-slate-600 transition-colors"
                  >
