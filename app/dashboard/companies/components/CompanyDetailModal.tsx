...
const DataPoint = ({ label, children }: { label: string; children: React.ReactNode }) => (
  <div>
    <p className="text-[10px] font-semibold normal-case tracking-normal text-slate-400">{label}</p>
  ...
  return (
    <div className="ms-modal-overlay">
      <section
        ...
        className="ms-modal w-full max-w-5xl"
      >
        <header className="ms-modal-header">
          ...
          <div className="flex items-start gap-3 pr-10">
            ...
            <div className="min-w-0">
              <p className="mb-1 text-xs font-medium text-slate-500">Company record</p>
              <h2 id="company-detail-title" className="ms-modal-title break-words">
        ...
        <div className="grid shrink-0 grid-cols-2 border-b border-slate-200 bg-slate-50 sm:grid-cols-4">
          <div className="border-r border-slate-200 px-5 py-4 sm:px-7">
            <p className="text-[10px] font-semibold normal-case tracking-normal text-slate-400">Active contacts</p>
            <p className="mt-1 text-2xl font-semibold text-slate-950">{contacts.length}</p>
          </div>
          <div className="px-5 py-4 sm:border-r sm:border-slate-200 sm:px-7">
            <p className="text-[10px] font-semibold normal-case tracking-normal text-slate-400">Holding group</p>
            <p className="mt-1 truncate text-sm font-semibold text-slate-800">{company.group?.name || 'Independent'}</p>
          </div>
          <div className="border-r border-t border-slate-200 px-5 py-4 sm:border-t-0 sm:px-7">
            <p className="text-[10px] font-semibold normal-case tracking-normal text-slate-400">Location</p>
            <p className="mt-1 truncate text-sm font-semibold text-slate-800">{company.city || 'Not recorded'}</p>
          </div>
          <div className="border-t border-slate-200 px-5 py-4 sm:border-t-0 sm:px-7">
            <p className="text-[10px] font-semibold normal-case tracking-normal text-slate-400">Company ID</p>
        ...
        <div className="min-w-0">
          <div className="grid gap-5 lg:grid-cols-2">
            <article className="ms-modal-section p-4">
              <div className="mb-5 flex items-center gap-2">
                <Landmark className="h-4 w-4 text-blue-600" />
                <h3 className="text-xs font-semibold normal-case tracking-normal text-slate-900">Company profile</h3>
              </div>
            ...
            <article className="ms-modal-section p-4">
              <div className="mb-5 flex items-center gap-2">
                ...
                <h3 className="text-xs font-semibold normal-case tracking-normal text-slate-900">Contact channels</h3>
              </div>
            ...
            <article className="ms-modal-section p-4 text-slate-800 lg:col-span-2">
              <div className="mb-4 flex items-center gap-2">
                <Server className="h-4 w-4 text-blue-600" />
                <h3 className="text-xs font-semibold normal-case tracking-normal text-slate-800">Infrastructure & hardware</h3>
              </div>
          ...
          <div className="mt-7">
            <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
              <div>
                <p className="text-[10px] font-semibold normal-case tracking-normal text-blue-600">People</p>
                <h3 className="mt-1 text-lg font-semibold text-slate-950">Active contacts</h3>
              </div>
              <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700">{contacts.length} people</span>
            </div>
              ...
              <div className="overflow-hidden rounded-lg border border-slate-200">
                <div className="max-h-72 overflow-auto">
                  <table className="w-full min-w-[680px] text-left text-sm">
                    <thead className="sticky top-0 z-10 bg-slate-50 text-[10px] font-semibold normal-case tracking-normal text-slate-400">
                      ...
                      {contacts.map(database => {
                        ...
                        return (
                          <tr key={database.id} className="transition hover:bg-blue-50/40">
                            <td className="px-4 py-3.5"><p className="font-semibold text-slate-900">{fullName}</p><p className="mt-0.5 text-xs text-slate-400">Database #{database.id}</p></td>
                            ...
                            <td className="px-4 py-3.5 text-right">
                              <button
                                ...
                                className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition hover:border-blue-200 hover:text-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500/30"
                                type="button"
                              >
